    ...
    chart = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                x: {
                    type: 'linear',
                    title: { display: true, text: 'x-axis', color: 'black' },
                    grid: {
                        drawBorder: false,
                        color: val => val.tick.value === 0 ? 'black' : '#ccc',
                ...
                y: {
                    type: 'linear',
                    title: { display: true, text: 'y-axis', color: 'black' },
                    grid: {
                        drawBorder: false,
                        color: val => val.tick.value === 0 ? 'black' : '#ccc',
                        lineWidth: val => val.tick.value === 0 ? 2 : 1,
        ...
        plugins: [{
            // Custom plugin to draw central x and y axis lines
            id: 'centerLines',
            beforeDraw(chart) {
...
chart.data.datasets = [
    ...
    {
        label: 'Roots',
        data: data.roots.includes("x₁")
            ? data.roots.match(/x₁ = ([\d.-]+), x₂ = ([\d.-]+)/).slice(1).map(r => ({x: parseFloat(r), y: 0}))
            : data.roots.includes("x =")
            ? [{x: parseFloat(data.roots.match(/x = ([\d.-]+)/)[1]), y: 0}]
            : [],
